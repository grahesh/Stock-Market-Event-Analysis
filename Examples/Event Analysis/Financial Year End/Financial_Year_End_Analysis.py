...
def getYearEndDates(timestamps):
        newTS=[]
        temp=timestamps[0].year
        
        for x in range(0, len(timestamps)-1):
            if(timestamps[x].year==temp and timestamps[x].month==4):
                newTS.append(timestamps[x-1])
                temp=timestamps[x].year+1
...
def findEvents(symbols, startday,endday, marketSymbol,verbose=False):
        ...
        endOfYear=getYearEndDates(timestamps)         
        ...
        # Event described is : Analysis Stock prices at the end of financial year.
...
eventProfiler.study(filename="FinancialYearEndEventStudy.jpg",plotErrorBars=True,plotMarketNeutral=True,plotEvents=False,marketSymbol='NSE500')
